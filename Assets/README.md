# 📂 Carpeta /Assets – Recursos Visuales y Materiales de Presentación

## 🎯 Propósito
La carpeta `/Assets` concentra los **recursos gráficos, mapas conceptuales, slides visuales, íconos y materiales de presentación** que apoyan la comunicación visual y narrativa del proyecto **CCIEAB – El Agaveño Burgers**.

---

## 📑 Contenido principal
- **Mapas conceptuales y diagramas** - Presentacion general del Guion General Mapa Emotivo 
  Representaciones visuales del modelo consultivo, flujos de trabajo y relaciones entre fases. 
  - **Slides visuales** - Presentacion Blueprint 
  Presentaciones ejecutivas por fase del proyecto (Inicio, Diagnóstico, Planeación, Implementación, Cierre). - 
- **Íconos y recursos gráficos** - material visual propio  
  Elementos visuales para reforzar identidad y claridad en materiales de comunicación.

---

## 🧭 Propósito estratégico
- **Comunicación visual:** facilitar comprensión de conceptos y procesos.  
- **Narrativa ejecutiva:** reforzar el tono profesional y emocional del proyecto.  
- **Defendibilidad:** respaldar la presentación con materiales claros y trazables.  

---

## 📂 Ejemplo de estructura interna
/Assets
├── Slide_Fase_I.pptx
├── Mapa_Conceptual_CCIEAB.png
├── Iconos_CCIEAB.zip
└── README.md

---

## 📝 Historial de commits (plantilla editable)
| **Fecha** | **Commit** | **Descripción ampliada** | **Responsable** |
|------------|-------------|---------------------------|-----------------|
| AAAA-MM-DD | `init/assets-readme` | Creación del README en `/Assets` para documentar recursos visuales del proyecto. | Equipo consultor |
| AAAA-MM-DD | `add/slide-fase-i` | Incorporación de presentación visual de la Fase I. | Equipo consultor |
| AAAA-MM-DD | `add/mapa-conceptual` | Integración del mapa conceptual del sistema CCIEAB. | Equipo consultor |
